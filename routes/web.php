...
';
		$langs_names = json_decode($langs_names, TRUE);
		$nulls = [];

		foreach (json_decode($langs, FALSE) as $code => $data) {
			$country = \App\Country::getCode($code);
			if ($country === NULL) {
				$nulls[] = $code;
				...
				try {
					...
					foreach ($data->languages as $l) {
						$languages[] = [
							'name'   => $langs_names[ $l ][ 'name' ],
							'native' => $langs_names[ $l ][ 'native' ],
							'code'   => $l,
						];
					}
